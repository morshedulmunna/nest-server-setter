...
export default class S3wrapper {
  ...
  public async getUploadSignedUrl(uploadFileName: string): Promise<string> {
    ...
    const presignedURL = await this._s3Client.getSignedUrlPromise(
      'putObject',
      params,
    );
  ...
  public async deleteFile(fileName: string): Promise<any> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const data = await this._s3Client.deleteObject(params).promise();
        console.log('object delete data = ', data);
        resolve(data);
      } catch (error: any) {
        console.log('buket object delete error = ', error.toString());
